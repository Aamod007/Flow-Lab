...
export const onExecuteWorkflow = async (nodesStr: string, edgesStr: string): Promise<{ success: boolean; message: string; logs: string[] }> => {
    const logs: string[] = []
            ...
            switch (currentNode.data.type) {
                case 'Slack':
                    ...
                    if (steps === 1) {
                        ...
                        // If it's an action, we send a message
                        // Need channel ID from metadata
                        const channelId = metadata.channelId || metadata.channel?.value
                        if (channelId) {
                            await postMessageToSlack(undefined, [{ label: 'Test', value: channelId }], String(currentInput))
                            logs.push(`- Sent Slack Message to ${channelId}`)
                        }
                        output = currentInput
                    }
                    break
